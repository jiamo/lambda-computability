...
| 943 | `M14-TQBF-PSPACE-HARD` | `P2` | `TODO_NEEDS_DESIGN` | M14-QBF-REACH-FORMULA, M13-SAVITCH-SPACE | docs/goal/evidence/M14-QBF-REACH-FORMULA.md | The formula half is done (M14-QBF-REACH-FORMULA): the midpoint recursion is a quantified Boolean formula of size polynomial in the width of a vertex and the depth. What is missing is the machine half: encoding the configurations of a machine of Start/SpaceMachine.lean running in space s as words of O(s + log n) bits, producing for each pair of blocks a formula of polynomial size that expresses one transition on those words, and showing that the whole formula is computed from the input in polynomial time - only then does every language in PSPACE reduce to TQBF. |
...
- Open boundary: The formula half is done (M14-QBF-REACH-FORMULA): the midpoint recursion is a quantified Boolean formula of size polynomial in the width of a vertex and the depth. What is missing is the machine half: encoding the configurations of a machine of Start/SpaceMachine.lean running in space s as words of O(s + log n) bits, producing for each pair of blocks a formula of polynomial size that expresses one transition on those words, and showing that the whole formula is computed from the input in polynomial time - only then does every language in PSPACE reduce to TQBF.
