...
| 351 | `M10-CWA-DEMOCRATIC` | `P2` | `DONE_WEAK` | M10-CWA-BICATEGORY | docs/goal/evidence/M10-CWA-DEMOCRATIC.md | Fullness and democracy are now defined (Cwa.IsFull, Cwa.IsDemocratic), the category of contexts of a full model is proved to have pullbacks (Cwa.IsFull.isPullback, Cwa.hasPullbacks_of_isFull), and the strictification of a category with pullbacks is proved full and, with a terminal object, democratic (Cwa.isFull_ofPullbacks, Cwa.isDemocratic_ofPullbacks), so the objects of Cwa.LcccModelCat do have both properties. What remains is the transport of a natural Pi-structure on a full model to a locally cartesian closed structure on its contexts - the existing proof LcccPullbacks.ofNaturalPiStruct is for the strictified model, where a type is a local universe, and the presentation Cwa.IsFull.ty is stable under substitution only up to isomorphism - and the comparison of a full democratic model with the strictification of its contexts as an equivalence in the lax 2-category. |
