...
| 959 | `M14-TQBF-IN-PSPACE` | `P1` | `TODO_NEEDS_DESIGN` | M14-QBF-WORD-LANG, M14-QBF-CODE-SPACE | docs/goal/evidence/M13-SAVITCH-SPACE.md | The space bound is now stated in the right currency: the evaluating stack machine of Start/Qbf.lean decides a closed formula within 2n^2 + 3n bits, n being the length of the code of the formula (M14-QBF-CODE-SPACE). What is missing is only the compilation of that stack machine into the offline machine of Start/SpaceMachine.lean - an offline machine that decodes the input word and performs the evaluation on its work tape within that bound - so Complexity.Space.PSPACE Complexity.Qbf.tqbfLang, and with it the completeness of TQBF, is still unavailable. |
...
- Status: `DONE_WEAK`
- Open boundary: Hardness is proved: Complexity.Qbf.QBF.pspaceHard_tqbfLang and .npspaceHard_tqbfLang reduce every language of (nondeterministic) polynomial space to Complexity.Qbf.tqbfLang by a single Cobham term of the input (Complexity.Qbf.QBF.redTerm, .eval_redTerm), which is a polynomial-time many-one reduction in the sense of Complexity.PolyManyOne. What is missing for completeness is the membership TQBF in PSPACE: the library has the memory bound of the evaluating stack machine (Complexity.Qbf.tqbf_memBits_le_size) but not a machine of Start/SpaceMachine.lean deciding tqbfLang in polynomial space, which is the open task M14-TQBF-IN-PSPACE; only with it can Complexity.Qbf.pspaceComplete_TQBF be stated.
