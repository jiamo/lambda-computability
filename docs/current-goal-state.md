...
| 351 | `M10-CWA-DEMOCRATIC` | `P2` | `DONE_WEAK` | M10-CWA-BICATEGORY | docs/goal/evidence/M10-CWA-DEMOCRATIC.md | Fullness and democracy are defined (Cwa.IsFull, Cwa.IsDemocratic), the category of contexts of a full model has pullbacks (Cwa.hasPullbacks_of_isFull), the strictification of a category with pullbacks is full and, with a terminal object, democratic, and - new - the category of contexts of a full model with a coherent substitution and a natural Pi-structure is locally cartesian closed (LcccPullbacks.ofIsFull, via Cwa.LcccOfFull.transpose and Cwa.LcccOfFull.transpose_naturality), so exit criteria 1 and 2 are met. The comparison of a full model with the strictification of its contexts is built as a morphism of models (Cwa.fullStrictify), the identity on contexts, bijective on terms (Cwa.fullStrictify_tmMap_bijective) and hitting every type up to an isomorphism of extended contexts over the base (Cwa.fullStrictify_essSurj). What remains is exit criterion 3, the equivalence in the lax 2-category: a morphism back would have to send a type over Gamma to a local universe whose base and generic family are independent of Gamma, which amounts to a universe for the model, and none is constructed here. |
...
- Title: An intrinsic description of the models presented by locally cartesian closed categories
- Status: `DONE_WEAK`
- Open boundary: Fullness and democracy are defined (Cwa.IsFull, Cwa.IsDemocratic), the category of contexts of a full model has pullbacks (Cwa.hasPullbacks_of_isFull), the strictification of a category with pullbacks is full and, with a terminal object, democratic, and - new - the category of contexts of a full model with a coherent substitution and a natural Pi-structure is locally cartesian closed (LcccPullbacks.ofIsFull, via Cwa.LcccOfFull.transpose and Cwa.LcccOfFull.transpose_naturality), so exit criteria 1 and 2 are met. The comparison of a full model with the strictification of its contexts is built as a morphism of models (Cwa.fullStrictify), the identity on contexts, bijective on terms (Cwa.fullStrictify_tmMap_bijective) and hitting every type up to an isomorphism of extended contexts over the base (Cwa.fullStrictify_essSurj). What remains is exit criterion 3, the equivalence in the lax 2-category: a morphism back would have to send a type over Gamma to a local universe whose base and generic family are independent of Gamma, which amounts to a universe for the model, and none is constructed here.
