...
| 170 | `M9-LAMBDAPI-LCCC` | `P1` | `BACKEND_PARTIAL` | M9-STLC-CCC | docs/goal/evidence/M9-LAMBDAPI-LCCC.md | Both sides are built, but the biequivalence is not claimed. Syntax: LambdaPiCwa.syntactic is a category with attributes whose types are the small types, LambdaPiCwa.weakPi its dependent product, and LambdaPiCwa.not_piStruct_weakPi shows eta fails. Semantics: the strictified model (Cwa.ofPullbacks) presents types by local universes, so substitution is strictly functorial, and a locally cartesian closed category carries there a dependent product with beta and eta (Cwa.piStructOfLccc). Universes: Cwa.Universe, SmallPi, PiClosed, CodePi and CodeSigma are what an interpretation needs; LambdaPiUniv.univ makes star a universe in the syntactic model and CwaUniv.universeOfHom makes every morphism one in the strictified model, and the two syntactic models are isomorphic (LambdaPiUniv.smallModelIso). A set-theoretic model is built (CwaTypeModel.model). Initiality is proved: LambdaPi.interp_exists_unique interprets every derivable judgement uniquely in any model with injective products, and LambdaPiInitial.mor packages it as a morphism of categories with attributes out of the syntax preserving the universe, its products and their codes. Not claimed: the biequivalence with locally cartesian closed categories, and closure of the universe of a general strictified model under the pushforward product on the nose - ruled out for a two-code universe of the standard model by CwaTypeNotClosed.not_piClosed. Its 1-categorical shadow is settled by M10-CWA-STRICT-FULL: strictification is full on underlying functors (Cwa.exists_pbCatHom_fnc) but not on the nose (Cwa.not_full_strictification), so the comparison is only up to isomorphism of types. Its 2-categorical shadow is settled by M10-CWA-LAX-BICAT: with lax 2-cells both sides are strict bicategories and strictification is compatible with whiskering in both dimensions, but it is still not packaged as a pseudofunctor, so the biequivalence remains out of reach. |
...
| 350 | `M10-CWA-BICATEGORY` | `P1` | `DONE_WEAK` | M10-LAMBDA-MODEL-REFLEXIVE | docs/goal/evidence/M10-CWA-BICATEGORY.md | What remains open is only the comparison with locally cartesian closed categories: the pseudofunctors relating models with Pi to locally cartesian closed categories, with an inverse up to equivalence, are not constructed, and no biequivalence is claimed. The rest is settled. The strict 2-cells give a strict bicategory of coherent models (Cwa.CModel.instBicategory), and out of the syntax of lambda-Pi they are rigid, so the syntactic model is bi-initial among the models of lambda-Pi with injective products (LambdaPiInitial.biInitial_syntacticModel) but not among all coherent models (not_biInitial_syntacticCModel), since a model need not have any types. With the strict 2-cells, strictification of a category with pullbacks cannot be made 2-functorial at all (M10-CWA-STRICT-RIGID, Cwa.isEmpty_twoCell_id_coyoneda), and it is full on the underlying functors but not on the nose (M10-CWA-STRICT-FULL), so a comparison has to weaken both the morphisms and the 2-cells. That weakening is now complete. With the lax 2-cells (M10-CWA-LAX-TWOCELL) the offending natural transformation does induce a 2-cell; a lax 2-cell is in fact exactly a natural transformation of the functors on contexts (M10-CWA-LAX-RIGID, Cwa.LaxTwoCell.equivNatTrans), so the interchange law holds in full generality (M10-CWA-LAX-INTERCHANGE) and the coherent models with lax 2-cells form a strict bicategory (M10-CWA-LAX-BICAT, Cwa.LaxCModel.instBicategory), alongside the 2-category of categories with pullbacks (Cwa.PbCat.instBicategory), strictification carrying whiskering to whiskering in both dimensions. The earlier reading of this boundary - that horizontal composition of lax 2-cells is missing and that interchange cannot even be stated with the present notion of lax 2-cell - is withdrawn as mistaken. Bi-initiality in the lax 2-category is stated in M10-CWA-LAX-BIINITIAL. |
...
| 514 | `M10-CWA-LAX-BIINITIAL` | `P1` | `DONE_WEAK` | M10-CWA-LAX-BICAT, M10-CWA-BICATEGORY | docs/goal/evidence/M10-CWA-LAX-BIINITIAL.md | The uniqueness clause for the models of lambda-Pi with injective products is not claimed. Both existence clauses hold (LambdaPiLaxBiInitial.nonempty_hom_lax and nonempty_laxTwoCell_modelHom), and by the classification of lax 2-cells uniqueness asks exactly that two interpretation functors out of the syntax admit at most one natural transformation (LambdaPiLaxBiInitial.subsingleton_laxTwoCell_iff_natTrans). The rigidity argument for strict 2-cells does not settle it: it forces the component at an extended context from the component at the base using the compatibility of a strict 2-cell with the types, which a bare natural transformation does not have, so the term component at an extension is unconstrained by naturality alone. Neither uniqueness nor a counterexample is proved. |
...
- Status: `DONE_WEAK`
- Open boundary: What remains open is only the comparison with locally cartesian closed categories: the pseudofunctors relating models with Pi to locally cartesian closed categories, with an inverse up to equivalence, are not constructed, and no biequivalence is claimed. The rest is settled. The strict 2-cells give a strict bicategory of coherent models (Cwa.CModel.instBicategory), and out of the syntax of lambda-Pi they are rigid, so the syntactic model is bi-initial among the models of lambda-Pi with injective products (LambdaPiInitial.biInitial_syntacticModel) but not among all coherent models (not_biInitial_syntacticCModel), since a model need not have any types. With the strict 2-cells, strictification of a category with pullbacks cannot be made 2-functorial at all (M10-CWA-STRICT-RIGID, Cwa.isEmpty_twoCell_id_coyoneda), and it is full on the underlying functors but not on the nose (M10-CWA-STRICT-FULL), so a comparison has to weaken both the morphisms and the 2-cells. That weakening is now complete. With the lax 2-cells (M10-CWA-LAX-TWOCELL) the offending natural transformation does induce a 2-cell; a lax 2-cell is in fact exactly a natural transformation of the functors on contexts (M10-CWA-LAX-RIGID, Cwa.LaxTwoCell.equivNatTrans), so the interchange law holds in full generality (M10-CWA-LAX-INTERCHANGE) and the coherent models with lax 2-cells form a strict bicategory (M10-CWA-LAX-BICAT, Cwa.LaxCModel.instBicategory), alongside the 2-category of categories with pullbacks (Cwa.PbCat.instBicategory), strictification carrying whiskering to whiskering in both dimensions. The earlier reading of this boundary - that horizontal composition of lax 2-cells is missing and that interchange cannot even be stated with the present notion of lax 2-cell - is withdrawn as mistaken. Bi-initiality in the lax 2-category is stated in M10-CWA-LAX-BIINITIAL.
