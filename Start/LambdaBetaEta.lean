...
/-- **Bridge to the abstract rewriting interface** (`Start/Rewriting.lean`): `betaEtaTStar` is the
reflexive–transitive closure of the composite relation. -/
theorem betaEtaTStar.iff_star {t u : Lambda} :
    betaEtaTStar t u ↔ Rewriting.Star betaEtaT t u := by
  constructor
  · intro h
    induction h with
  ...
  · intro h
    induction h with
    | refl => exact betaEtaTStar.refl t
    | tail _ hs ih => exact ih.tail hs

/-- The strip lemma for the composite relation, an instance of `Rewriting.strip`. -/
theorem betaEtaTStar.strip {t u v : Lambda} (h₁ : betaEtaT t u) (h₂ : betaEtaTStar t v) :
    ∃ w, betaEtaTStar u w ∧ betaEtaT v w := by
  obtain ⟨w, hw₁, hw₂⟩ :=
    Rewriting.strip (fun _ _ _ k₁ k₂ => betaEtaT.diamond k₁ k₂) h₁ (betaEtaTStar.iff_star.1 h₂)
  exact ⟨w, betaEtaTStar.iff_star.2 hw₁, hw₂⟩

/-- The composite relation is confluent, an instance of `Rewriting.confluent_of_diamond`. -/
theorem betaEtaTStar.confluent {t u v : Lambda} (h₁ : betaEtaTStar t u) (h₂ : betaEtaTStar t v) :
    ∃ w, betaEtaTStar u w ∧ betaEtaTStar v w := by
  obtain ⟨w, hw₁, hw₂⟩ :=
    Rewriting.confluent_of_diamond (fun _ _ _ k₁ k₂ => betaEtaT.diamond k₁ k₂)
      (betaEtaTStar.iff_star.1 h₁) (betaEtaTStar.iff_star.1 h₂)
  exact ⟨w, betaEtaTStar.iff_star.2 hw₁, betaEtaTStar.iff_star.2 hw₂⟩
...
/-- The bridge for βη-reduction: it is the closure of `betaEtaStep`. -/
theorem betaEtaReduces.iff_star {t u : Lambda} :
    betaEtaReduces t u ↔ Rewriting.Star betaEtaStep t u := by
  constructor
  · intro h
    induction h with
    | refl => exact Rewriting.Star.refl t
  ...
  · intro h
    induction h with
    | refl => exact betaEtaReduces.refl t
    | tail _ hs ih => exact ih.tail hs

/-- βη-reduction is confluent, in the vocabulary of `Start/Rewriting.lean`. -/
theorem confluent_betaEtaStep : Rewriting.Confluent betaEtaStep := by
  intro t u v h₁ h₂
  obtain ⟨w, hw₁, hw₂⟩ :=
    betaEta_church_rosser (betaEtaReduces.iff_star.2 h₁) (betaEtaReduces.iff_star.2 h₂)
  exact ⟨w, betaEtaReduces.iff_star.1 hw₁, betaEtaReduces.iff_star.1 hw₂⟩

/-- The bridge for βη-conversion: it is the conversion generated by `betaEtaStep`. -/
theorem betaEtaConv.iff_conv {t u : Lambda} :
    betaEtaConv t u ↔ Rewriting.Conv betaEtaStep t u := by
  constructor
  · intro h
    induction h with
    | refl => exact Rewriting.Conv.refl t
    | step _ hs ih => exact Rewriting.Star.tail ih (Or.inl hs)
    | symm_step _ hs ih => exact Rewriting.Star.tail ih (Or.inr hs)
  · intro h
    induction h with
    | refl => exact betaEtaConv.refl t
    | tail _ hs ih =>
        rcases hs with hs | hs
        · exact ih.step hs
        · exact ih.symm_step hs

/-- **Confluence in the form of the Church–Rosser property**: βη-convertible terms have a
common βη-reduct.  An instance of `Rewriting.conv_iff_joins_of_confluent`. -/
theorem betaEtaConv.common_reduct {t u : Lambda} (h : betaEtaConv t u) :
    ∃ w, betaEtaReduces t w ∧ betaEtaReduces u w := by
  obtain ⟨w, hw₁, hw₂⟩ :=
    (Rewriting.conv_iff_joins_of_confluent confluent_betaEtaStep).1 (betaEtaConv.iff_conv.1 h)
  exact ⟨w, betaEtaReduces.iff_star.2 hw₁, betaEtaReduces.iff_star.2 hw₂⟩
