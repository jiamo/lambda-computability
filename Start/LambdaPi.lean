...
theorem Pars.confluent {t t₁ t₂ : Tm} (h₁ : Pars t t₁) (h₂ : Pars t t₂) :
  ...
  induction h₁ with
  | refl => exact ⟨t₂, h₂, Pars.refl _⟩
  | @tail u v _ hs ih =>
      obtain ⟨w, hw₁, hw₂⟩ := ih
      obtain ⟨z, hz₁, hz₂⟩ := Pars.strip hs hw₁
...
theorem church_rosser {t t₁ t₂ : Tm} (h₁ : Red t t₁) (h₂ : Red t t₂) :
  ...
  obtain ⟨u, hu₁, hu₂⟩ := (Pars.ofRed h₁).confluent (Pars.ofRed h₂)
  exact ⟨u, hu₁.red, hu₂.red⟩

/-- Conversion is exactly joinability. -/
theorem Conv.church_rosser {t u : Tm} (h : Conv t u) : ∃ v, Red t v ∧ Red u v := by
  induction h with
  | refl => exact ⟨t, Red.refl t, Red.refl t⟩
  | @step u v _ hs ih =>
      obtain ⟨w, hw₁, hw₂⟩ := ih
      obtain ⟨z, hz₁, hz₂⟩ := LambdaPi.church_rosser hw₂ (Red.single hs)
      exact ⟨z, hw₁.trans hz₁, hz₂⟩
  | @stepInv u v _ hs ih =>
      obtain ⟨w, hw₁, hw₂⟩ := ih
      exact ⟨w, hw₁, Red.head hs hw₂⟩
