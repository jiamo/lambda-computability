...
theorem haltInp_eq (n : ℕ) :
  ...
  rw [haltInp, range_map_const, haltCode]
...
theorem partrec_of_tm2ComputableInPolyTime {α β αΓ βΓ : Type}
  ...
  @partrec_of_tm2Computable α β αΓ βΓ ea eb f h.toTM2ComputableInTime.toTM2Computable inst

/-- The step bound of `Mathlib`'s polynomial-time class, read at the level of codes. -/
theorem haltsWithin_of_tm2ComputableInPolyTime {α β αΓ βΓ : Type}
    {ea : α → List αΓ} {eb : β → List βΓ} {f : α → β}
    (h : Turing.TM2ComputableInPolyTime ea eb f) [∀ k, Fintype (h.tm.Γ k)] (a : α) :
    HaltsWithin h.tm ((List.map h.inputAlphabet.invFun (ea a)).map (encG h.tm))
